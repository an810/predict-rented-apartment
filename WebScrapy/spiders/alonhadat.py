...
class AlonhadatSpider(scrapy.Spider):
    ...
    def __del__(self):
        ...
        # try:
        #     if hasattr(self, "logger"):  # Ensure logger exists before using it
        #         self.logger.info("Spider shutting down")
        # except Exception:
        #     pass  # Ignore logging error during shutdown
    ...
    def parse_info(self, response):
        ...
        phone_number_element = response.css('div.contact-info div.fone a::text').get()
        phone_number: str = phone_number_element.replace('.', '').replace(' ', '') if phone_number_element else 'N/A'
